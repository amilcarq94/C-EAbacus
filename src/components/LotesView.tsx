...
export const LotesView: React.FC<LotesViewProps> = ({
  ...
  // Resumen dinámico de existencias agrupado por especie
  const stockSummaryByEspecie = useMemo(() => {
    const map: Record<string, { especie: string; totalBolsas: number; totalKg: number; countLotes: number }> = {};
    let grandTotalBolsas = 0;
    let grandTotalKg = 0;
    ...
    filteredLotes.forEach(lote => {
      ...
      if (!map[esp]) {
        map[esp] = { especie: esp, totalBolsas: 0, totalKg: 0, countLotes: 0 };
      }
      map[esp].totalBolsas += bolsas;
      map[esp].totalKg += totalKgLote;
      map[esp].countLotes += 1;

      grandTotalBolsas += bolsas;
      grandTotalKg += totalKgLote;
    });

    const groups = Object.values(map).sort((a, b) => b.totalKg - a.totalKg);
  ...
  return (
    <div className="space-y-6" id="lotes-view-container">
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-3" id="stock-by-especie-summary-panel">
        <div className="flex flex-wrap justify-between items-center gap-2 border-b border-gray-100 pb-2.5">
          <div className="flex items-center gap-2 text-xs font-bold text-[#00603C] uppercase tracking-wider">
            <Sprout className="w-4.5 h-4.5 text-[#00603C]" />
            <span>Resumen de Existencias por Especie</span>
            {hasActiveFilters && (
          ...
          <div className="text-xs text-gray-500 font-medium flex items-center gap-2">
            ...
            <span className="font-extrabold text-[#00603C] bg-[#E3EFE7] px-2.5 py-0.5 rounded-md font-mono">
              {formatBolsasLabel(stockSummaryByEspecie.grandTotalBolsas)}
            </span>
            <span className="font-extrabold text-gray-800 bg-gray-100 px-2.5 py-0.5 rounded-md font-mono">
              {formatKg(stockSummaryByEspecie.grandTotalKg)}
            </span>
          </div>
        </div>

        {stockSummaryByEspecie.groups.length === 0 ? (
          <div className="text-xs text-gray-400 py-4 text-center bg-gray-50/60 rounded-xl border border-dashed border-gray-200">
            No hay existencias registradas que coincidan con los filtros aplicados.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {stockSummaryByEspecie.groups.map(group => {
              const porcentaje = stockSummaryByEspecie.grandTotalKg > 0 
                ? ((group.totalKg / stockSummaryByEspecie.grandTotalKg) * 100).toFixed(1) 
                : '0';
              return (
                <div 
                  key={`especie-${group.especie}`}
                  className="bg-gradient-to-br from-[#00603C]/5 via-emerald-50/30 to-white p-3.5 rounded-xl border border-[#00603C]/20 hover:border-[#00603C]/50 hover:shadow-2xs transition-all flex flex-col justify-between"
                >
                  <div className="flex justify-between items-center gap-1.5 mb-2">
                    <span className="text-xs font-black text-[#00603C] uppercase tracking-wide truncate flex items-center gap-1.5" title={group.especie}>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between items-center text-[10px] text-gray-400 border-t border-gray-100/60 pt-1">
                      <span>Proporción:</span>
                      <span className="font-bold text-emerald-800 font-mono">{porcentaje}%</span>
                    </div>
                  </div>
                </div>
              );
            ...
            {/* Ficha Resumen Total Acumulado General cuando hay más de 1 especie */}
            {stockSummaryByEspecie.groups.length > 1 && (
              <div className="bg-[#00603C] text-white p-3.5 rounded-xl border border-[#00603C] shadow-2xs flex flex-col justify-between">
                <div className="flex justify-between items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] font-bold text-[#00603C] bg-white px-2 py-0.5 rounded-md shrink-0 font-mono">
                    {stockSummaryByEspecie.totalLotes} {stockSummaryByEspecie.totalLotes === 1 ? 'lote' : 'lotes'}
                  </span>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between items-baseline text-xs border-t border-white/20 pt-1">
                    <span className="text-white/80 font-medium">Stock Total Kg:</span>
