...
export const BatchPrintLotesModal: React.FC<BatchPrintLotesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xs flex flex-col items-center justify-start overflow-y-auto p-2 sm:p-6 animate-in fade-in duration-200 print:p-0 print:bg-white print:static print:inset-auto print:z-auto">
      ...
      <div className="w-full max-w-4xl bg-slate-900 text-white px-5 py-3.5 rounded-2xl shadow-2xl border border-slate-800 flex items-center justify-between mb-6 sticky top-2 z-50 print:hidden">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-[#00603C] rounded-xl text-white">
            <Printer className="w-5 h-5 text-[#C9922E]" />
          </div>
          <div>
            <h3 className="font-serif font-bold text-sm tracking-wide uppercase text-white">
              Vista de Impresión de Fichas de Lote
            </h3>
            <p className="text-xs text-slate-300">
              Se imprimirá <strong className="text-amber-400 font-mono font-bold">{lotes.length}</strong> {lotes.length === 1 ? 'ficha técnica' : 'fichas técnicas (1 por hoja)'}.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 px-5 py-2 bg-[#00603C] hover:bg-[#254731] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md transition cursor-pointer"
          >
            <Printer className="w-4 h-4 text-[#C9922E]" />
            <span>Imprimir {lotes.length === 1 ? 'Ficha' : `las ${lotes.length} Fichas`}</span>
          </button>
          <button
            onClick={onClose}
